...
async function edit(_id, change) {
    const order = await get(_id); // Fetch the existing order
    ...
    Object.keys(change).forEach((key) => {
      order[key] = change[key]; // Update order properties dynamically
    });
  
    await order.save(); // Save the updated order
    await order.populate("products"); // Populate product details
